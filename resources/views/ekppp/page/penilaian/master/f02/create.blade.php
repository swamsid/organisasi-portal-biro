        ...
        <div class="col-12 col-lg-12">
            ...
            <div class="card mb-4">
                <div class="card-header">
                    <h5 class="card-tile mb-0">Form data Master F02</h5>
                </div>
                <div class="card-body">
                    @if (Route::is('pekppp.master.master-f02.create'))
                        <form action="{{ route('pekppp.master.master-f02.store') }}" method="post"
                            enctype="multipart/form-data">
                        @else
                            <form action="{{ route('pekppp.master.master-f02.update', $master->id) }}" method="post"
                                enctype="multipart/form-data">
                                @method('PUT')
                    @endif
                    @csrf
                    ...
                    @if (!Route::is('pekppp.master.master-f02.create'))
                        <div class="row mb-3 d-flex align-items-end">
                            @foreach ($master->jawaban as $items)
                                ...
                                <div class="col-7">
                                    <label class="form-label" for="ecommerce-product-sku">Skala</label>
                                    <input type="text" class="form-control" name="jawaban[]" id="jawaban" value="{{ $items->jawaban }}" required>
                                </div>
                                <div class="col-1">
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (e.value === true) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ url('pekppp/master/hapus-detailf02') }}/" + id,
                        data: {
                            _token: CSRF_TOKEN,
                            id: id
                        },
